...
(() => {
  ...
  function t() {
    ...
    return {
      ...
      textMuted: d ? "#85858D" : "#3C3C44",
      // WCAG fix: was #4A4A52 (2.06:1 dark) / #ABABB4 (2.09:1 light)
      accent: d ? "#8B6CFF" : "#6E54F5",
      // WCAG fix: was #7B61FF (4.30:1 dark / 3.86:1 light)
      accentEnd: "#00C2FF",
      accentAI: d ? "#FF8C42" : "#A85C2C",
      // WCAG fix light: was #FF8C42 (2.12:1 on light bg)
      success: d ? "#34C759" : "#188236",
      // WCAG fix light: was #1A8C3A (3.97:1)
  ...
  async function callClaudeAPI(apiKey, structure) {
    var _a, _b, _c;
    ...
    return ((_c = (_b = (_a = data == null ? void 0 : data.content) == null ? void 0 : _a[0]) == null ? void 0 : _b.text) == null ? void 0 : _c.trim()) || null;
  }
  async function callClaudeBatch(apiKey, nodes) {
    var _a, _b, _c;
...
JSON output:`;
    ...
    const data = await res.json();
    const text = ((_c = (_b = (_a = data == null ? void 0 : data.content) == null ? void 0 : _a[0]) == null ? void 0 : _b.text) == null ? void 0 : _c.trim()) || "";
    try {
  ...
  async function callClaudeContext(apiKey, nodes, textSamples, scanOnly = false) {
    var _a, _b, _c;
...
${JSON.stringify(nodes, null, 2)}`;
  ...
  function buildSinglePrompt(structure) {
...
Name:`;
  ...
  function renderHealthPanel(c) {
    ...
    const pctLabel = pct >= 0 ? '<span style="font-size:11px;font-weight:600;color:' + color + ';">' + pct + "%</span>" : '<span style="font-size:11px;color:' + c.textMuted + ';">\u2014</span>';
    const chevron = expanded ? '<svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M2 4L5 7L8 4" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>' : '<svg width="10" height="10" viewBox="0 0 10 10" fill="none"><path d="M4 2L7 5L4 8" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>';
    const header = [
    ...
    const barHtml = h !== null ? '<div class="sr-health-bar-wrap"><div class="sr-health-bar-fill" style="width:' + pct + "%;background:" + color + ';"></div></div>' : '<div class="sr-health-bar-wrap"><div class="sr-health-bar-fill" style="width:0%;background:' + c.border + ';"></div></div>';
    const labelHtml = isCheckingHealth ? '<div style="font-size:11px;color:' + c.textMuted + ';font-style:italic;margin:4px 0 8px;">Scanning\u2026</div>' : h === null ? '<div style="font-size:11px;color:' + c.textMuted + ';font-style:italic;margin:4px 0 8px;">Click Check Layer Health to scan</div>' : allClean ? '<div style="font-size:11px;color:' + c.success + ';margin:4px 0 8px;">\u2713 All ' + h.total + " layers are named</div>" : '<div style="font-size:11px;color:' + c.textSub + ';margin:4px 0 8px;">' + h.unnamed + " of " + h.total + " layers need renaming</div>";
  ...
  function renderLogOverlay(c) {
    const items = state.log.length === 0 ? `<div class="sr-log-empty">Nothing renamed yet \u2014 start designing!</div>` : state.log.map((entry, i) => {
      const isNew = i === 0;
  ...
  function renderFull() {
    ...
    if (state.isAnalyzing) {
      dotClass = "dot-ai";
      statusLabel = `Analyzing${state.analyzeProgress ? " \xB7 " + state.analyzeProgress : "\u2026"}`;
    ...
    <div class="plugin-wrap" role="main">
        ...
        <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
          <div style="flex:1;min-width:0;">
            ...
            <div style="font-size:11px;color:${c.textSub};margin-top:2px;line-height:1.4;">
              ${!mainToggleOn ? "Paused \u2014 toggle to start renaming" : aiToggleOn ? "Renaming with Claude AI as you design" : "Renaming with local rules as you design"}
            </div>
          </div>
          <button
            class="sr-toggle"
          ...
          <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:10px;">
            <div style="flex:1;min-width:0;">
              ...
              <div style="font-size:10px;color:${c.textSub};margin-top:2px;">
                ${!state.hasApiKey ? `<span style="color:${c.textMuted};">Add API key to enable</span>` : aiToggleOn ? `<span style="color:${c.accentAI};">\u25CF Active \u2014 Claude Haiku</span>` : `<span style="color:${c.textMuted};">Off \u2014 using local rules</span>`}
              </div>
            </div>
            <button
              class="sr-toggle"
      ...
      <div class="sr-log-wrap">
        ...
        <div class="sr-log-list" role="list" aria-label="Recent renames">
          ${state.log.length === 0 ? `<div class="sr-log-empty">Nothing renamed yet \u2014 start designing!</div>` : logItems}
        </div>
      </div>
  ...
  window.showTooltip = (anchor, text) => {
  ...
  var _authorHideTimer = null;
  window.showAuthorTooltip = () => {
    if (_authorHideTimer) {
      clearTimeout(_authorHideTimer);
      _authorHideTimer = null;
    }
    const el = document.getElementById("sr-author-tooltip");
    if (el) el.style.display = "block";
  };
  window.keepAuthorTooltip = () => {
    if (_authorHideTimer) {
      clearTimeout(_authorHideTimer);
      _authorHideTimer = null;
    }
  };
  window.hideAuthorTooltip = () => {
  ...
  window.handleSaveKey = () => {
    var _a, _b;
    const val = (_b = (_a = document.getElementById("apiKeyInput")) == null ? void 0 : _a.value) == null ? void 0 : _b.trim();
    if (!val) return;
  ...
  window.handleTestKey = () => {
    const statusEl = document.getElementById("test-status");
    if (statusEl) {
      statusEl.textContent = "Testing\u2026";
      statusEl.style.color = t().accent;
    }
    parent.postMessage({ pluginMessage: { type: "getApiKey" } }, "*");
  };
  window.runApiTest = async (apiKey) => {
    var _a, _b, _c;
    const statusEl = document.getElementById("test-status");
    const c = t();
    try {
      ...
      const data = await res.json();
      if (res.ok && ((_b = (_a = data == null ? void 0 : data.content) == null ? void 0 : _a[0]) == null ? void 0 : _b.text)) {
        if (statusEl) {
          statusEl.textContent = "\u2713 Connected \u2014 key works!";
          statusEl.style.color = c.success;
        }
      } else {
        const errMsg = ((_c = data == null ? void 0 : data.error) == null ? void 0 : _c.message) || `HTTP ${res.status}`;
        if (statusEl) {
  ...
  window.onmessage = async (event) => {
    ...
    if (msg.type === "contextNodes") {
      window._pendingContextNodes = msg.nodes;
      window._pendingContextTexts = msg.textSamples;
      window._pendingScreenName = msg.screenName || "Screen";
      window._pendingScreenNodeId = msg.screenNodeId || null;
      window._pendingScanOnly = !!msg.scanOnly;
      if (!msg.scanOnly) {
    ...
    if (msg.type === "apiKeyForContext") {
      const nodes = window._pendingContextNodes;
      const texts = window._pendingContextTexts || [];
      const screenName = window._pendingScreenName || "Screen";
      const screenNodeId = window._pendingScreenNodeId || null;
      if (!nodes) return;
      if (!msg.apiKey) {
        const fromScan = window._pendingScanOnly;
        if (fromScan) {
          state.isAnalyzing = false;
          stopLoading();
        } else {
      ...
      try {
        ...
        render();
        const scanOnly = !!window._pendingScanOnly;
